...
describe('envelope encryption', () => {
    ...
    test('tampered payload ciphertext fails', () => {
        ...
        expect(() => decryptTxSecureRecord({ ...record, payload_ct: tampered }, parseMasterKeyHex(mkHex()))).toThrow(/decrypt_failed|invalid_hex|invalid_length/);
    });
    test('tampered payload tag fails', () => {
        const record = makeRecord();
        const tampered = record.payload_tag.slice(0, -2) + (record.payload_tag.slice(-2) === '00' ? '01' : '00');
        expect(() => decryptTxSecureRecord({ ...record, payload_tag: tampered }, parseMasterKeyHex(mkHex()))).toThrow(/decrypt_failed|invalid_hex|invalid_length/);
    });
